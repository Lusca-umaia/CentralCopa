...
async function generateGames() {
    ...
    let GamesSelection = objGames.filter((item) => item.timeA == 'Argentina');
    let GamesSelectionTwo = objGames.filter((item) => item.timeB == 'Argentina');
    for (let index = 0; index < GamesSelectionTwo.length; index++) {
        GamesSelection.push(GamesSelectionTwo[index]);
    }
    let qualifyingGames = GamesSelection.filter((item) => item.fase == "Classificatória");
    createBoxClassification(qualifyingGames, Qualifying);
    if (GamesSelection.length > 3) {
        //octave
        let octaveGame = GamesSelection.filter((item) => item.fase == "Oitavas-Final");
        if (octaveGame.length == 1) {
            createBox(octaveGame, Container);
        }
        //quarter
        let quarterGame = GamesSelection.filter((item) => item.fase == "Quartas-Final");
        if (quarterGame.length == 1) {
            createBox(quarterGame, Container);
        }
        //Semi
        let semiGame = GamesSelection.filter((item) => item.fase == "Semi-Final");
        if (semiGame.length == 1) {
            createBox(semiGame, Container);
        }
        //Final
        let finalGame = GamesSelection.filter((item) => item.fase == "Final");
...
function createBoxClassification(qualifyingGames, qualifying) {
    for (let index = 0; index < qualifyingGames.length; index++) {
        ...
        let section = document.getElementById('contQualifying');
        let matchCollaps = document.getElementById('matchCollaps');
